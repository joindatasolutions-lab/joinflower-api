...
USERS = {
    "flora.admin": {
        "password": os.getenv("TEST_PASSWORD_FLORA_ADMIN", "Junior2018*:2"),
        "expected_role": "Admin",
        "allowed_modules": {"pedidos", "produccion", "domicilios", "inventario", "reportes", "usuarios"},
    },
    "flora.pedidos": {
        "password": os.getenv("TEST_PASSWORD_FLORA_PEDIDOS", "FloraPedidos2026*"),
        "expected_role": "PEDIDOS",
        "allowed_modules": {"pedidos"},
    },
    "flora.florista1": {
        "password": os.getenv("TEST_PASSWORD_FLORA_FLORISTA1", "FloraFlorista12026*"),
        "expected_role": "FLORISTA",
        "allowed_modules": {"produccion"},
    },
    "flora.domi1": {
        "password": os.getenv("TEST_PASSWORD_FLORA_DOMI1", "FloraDomi12026*"),
        "expected_role": "DOMICILIARIO",
        "allowed_modules": {"domicilios"},
    },
    "flora.inventario": {
        "password": os.getenv("TEST_PASSWORD_FLORA_INVENTARIO", "FloraInventario2026*"),
        "expected_role": "Inventarista",
...
def test_role_module_access_matrix(login: str):
    ...
    me = me_resp.json()
    assert int(me["empresaID"]) == FLORA_EMPRESA_ID
